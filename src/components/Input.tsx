...
export function Popover({ value, isActive = false }: { value: string, isActive?: boolean}) {
...
export default function Input({labelName, labelFor, inputType, inputName, getInputValue, isSentForm, error}: InputProps) {
    ...
    useEffect(() => {
        if (input && input.name === 'signup_password_field' && isSelected && !input.value.length) setIsActivePopover(true);
    ...
    return (
        <div className={`relative input-container`}>
            ...
            <input ref={customInput} className={`w-full pl-3 ${inputName.includes('password') ? 'pr-16 sm:pr-20' : 'pr-3'} py-2 border-1 border-[#ffffff80] focus:border-white rounded-md outline-none`} type={type} name={inputName} id={labelFor} onFocus={() => setIsSelected(true)} onBlur={() => setIsSelected(false)} onChange={getValue}/>
            ...
            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex gap-2">
                ...
                <>
                    ...
                    <Popover isActive={isActivePopover} value={value}/>
                </>
                }
